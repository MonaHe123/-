...
// first, set up the translation 
    ...
    pageTable = new TranslationEntry[numPages];
    for (i = 0; i < numPages; i++) {
	pageTable[i].virtualPage = i;	// for now, virtual pagVe # = phys page #
	//pageTable[i].physicalPage = i;
	pageTable[i].physicalPage = machine->AllocateNewProgram();	
...
#endif
// zero out the entire address space, to zero the unitialized data segmeHnt 
// and the stack segment
   // bzero(machine->mainMemory, size);
	...
	int pos_vm = noffH.code.virtualAddr;
	char current_byte;
	for(i = 0; i < noffH.code.size; i++)
	{
        	executable->ReadAt(&current_byte, 1, pos++);
		file->WriteAt(&current_byte, 1, pos_vm++);
	}
    }
