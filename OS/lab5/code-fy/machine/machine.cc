...
for (int i = 0; i < NumPhysPages; i++)	
	{
		...
		{
			bitMap[i] = 1;			
			printf("Physical page number %d has been allocated.\n", i);			
			return i;
		}		
	}
	return -1;
...
Machine::ClearCurrentProgram()
{
	...
	for (int i = 0; i < NumPhysPages; i++)
	{
		if (rPageTable[i].threadID == currentThread->GetThreadID())
		{
			bitMap[i] = 0;
			printf("Physical page number %d has been cleared, thread ID is %d.\n", i, rPageTable[i].threadID);
		}		
	}
#else
	for (int i = 0; i < pageTableSize; i++)	
	{			
		int k = pageTable[i].physicalPage;
		if (bitMap[k] == 1)
		{
			bitMap[k] = 0;
			printf("Physical page number %d has been cleared.\n", k);
		}
	}
#endif
}
