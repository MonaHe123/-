...
{
    int i;
    for (i = 1; i <= MAX_THREAD_NUMBER; i++)
    {
	if (ThreadID_Array[i-1] == 0)
	break;
    }
    if (i <= MAX_THREAD_NUMBER)
    {
        ThreadID_Array[i-1] = 1;
        return i;
    }
    else 
        return -1;
...
Thread::Thread(char* threadName)
{
    ...
    if (ThreadID == -1)
        printf("Thread number exceeds the limit 128!");
    ASSERT(ThreadID != -1);
...
Thread::Yield ()
{
    ...
    if (nextThread != NULL) {
	scheduler->ReadyToRun(this); //put the old thread into the readylist
	scheduler->Run(nextThread);  //context switch
    }
    (void) interrupt->SetLevel(oldLevel);
}
