...
Directory::GetType(char* file_name)
{
	for (int i = 0; i < tableSize; i++)
        if (table[i].inUse && !strncmp(table[i].name, file_name, FileNameMaxLen))
	    return table[i].type;
...
{ 
    char file_name[FileNameMaxLen + 1];     //name : haha/haha1
	int pos = -1;
	...
	for (int i = pos; i < strlen(name); i++)
        file_name[j++] = name[i];   //file_name: haha
	file_name[j] = '\0';
    	if (FindIndex(name) != -1)
		return FALSE;
    	for (int i = 0; i < tableSize; i++)
...
void
Directory::Print(int k)
{ 
	if (k == 0)
		printf("Root Directory, Sector: 1\n"); 
	for (int j = 0; j < k; j++)
		printf("	");
	...
	if (table[i].inUse) 
	{
		...
		if (table[i].type == 0)  //is directory
		{
			for (int j = 0; j <= k; j++)
				printf("	");
			printf("Subdirectory name: %s, Sector: %d\n",table[i].name, table[i].sector);
			OpenFile* subDirectory = new OpenFile(table[i].sector);
