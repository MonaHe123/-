...
FileSystem::Create(char *name, int initialSize)
{
    ...
    int name_sector = directory->FindDirectory(name);
	...
	{	
		...
		{	
			if (initialSize == -1) //directory
			{		
				if (!directory->Add(name, sector, 0))
           			 	return FALSE;	// no space in directory
    	    			hdr = new FileHeader;
				initialSize = DirectoryFileSize;
	    			if (!hdr->Allocate(freeMap, initialSize))            						return  FALSE;	// no space on disk for data
	   		 	success = TRUE;
...
{ 
    ...
    directory = new Directory(NumDirEntries);
    directory->FetchFrom(directoryFile);
    dir_sector = directory->FindDirectory(name);
    directory = new Directory(NumDirEntries);
	printf("dir_sector:%d\n", dir_sector);
    if (dir_sector >= 0)
	...
	if (synchDisk->numThreads[sector])
	{
		printf("Existing threads still hold on it, can not remove.\n");
		return false;
	}
	...
	{
		printf("Directory still contains files.");		
		return false;
	}
...
int 
FileSystem::ReadPipe(char* data, int sendID, int receiveID)
{
	printf("Read pipe.\n");
	char name[10];
	int pos = 0;
	if (sendID / 100 > 0)
	{
		name[pos++] = '0'+ sendID / 100;
		sendID = sendID % 100;
	} 
	if (sendID / 10 > 0)
	{
		name[pos++] = '0'+ sendID / 10;
		sendID = sendID % 10;
	}
	...
	if (receiveID / 100 > 0)
	{
		name[pos++] = '0'+ receiveID / 100;
		receiveID = receiveID % 100;
	} 
	if (sendID / 10 > 0)
	{
		name[pos++] = '0'+ receiveID / 10;
		receiveID = receiveID % 10;
	}
	name[pos++] = '0'+ receiveID;
	name[pos] = '\0';
	
	OpenFile *pipe = Open(name);
	ASSERT(pipe != NULL);
	printf("Open pipe: %s\n", name);
	int sector = pipe->fileHeaderSector;
	FileHeader *pipehdr;
	pipehdr = new FileHeader;
	pipehdr->FetchFrom(sector);
	int length = pipehdr->numBytes;
	pipehdr->WriteBack(sector);
...
void
FileSystem::WritePipe(char* data, int length, int sendID, int receiveID)
{
	printf("Write pipe.\n");
	char name[10];
	int pos = 0;
